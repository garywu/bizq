...
// .wrangler/tmp/bundle-Pp6EdD/checked-fetch.js
...
var RateLimiter = class {
  ...
  async checkLimit(clientIP, maxRequests = 50, windowMs = 36e5) {
    ...
    try {
      ...
      requests.push(now);
      await this.cache.put(key, JSON.stringify(requests), { expirationTtl: windowMs / 1e3 });
      return true;
    } catch (error) {
...
var AIDomainGenerator = class {
  static {
    __name(this, "AIDomainGenerator");
  }
  openai;
  constructor(apiKey) {
    this.openai = new openai_default({ apiKey });
  }
  async generateDomains(industry, keywords = [], limit2 = 10) {
    try {
      const keywordsStr = keywords.length > 0 ? ` with keywords: ${keywords.join(", ")}` : "";
      const prompt = `Generate ${limit2} creative and memorable domain name suggestions for a ${industry} business${keywordsStr}.

Requirements:
- Suggest only the domain names without any TLD extension (.com, .io, etc.)
- Make them unique, brandable, and easy to remember
- Avoid hyphens when possible, but use them if it improves memorability
- Consider industry relevance and current trends
...
Example format: ["DomainName1", "DomainName2", "DomainName3"]`;
      ...
      try {
        ...
        if (Array.isArray(suggestions)) {
          return suggestions.filter((s) => typeof s === "string" && s.trim().length > 0).map((s) => s.trim()).slice(0, limit2);
        }
      } catch (parseError) {
        ...
        return suggestions.slice(0, limit2);
      }
      return [];
    } catch (error) {
...
var DomainAvailabilityChecker = class {
  static {
    __name(this, "DomainAvailabilityChecker");
  }
  async checkDomain(domain, tld = "com") {
    try {
      const fullDomain = `${domain}.${tld}`;
      const response = await fetch(`https://domain-availability.whoisxmlapi.com/api/v1?apiKey=demo&domainName=${fullDomain}`, {
        method: "GET",
        headers: {
          "Accept": "application/json"
        }
      ...
      if (response.ok) {
        ...
        return data.domainAvailability === "AVAILABLE";
      }
      console.warn(`WHOIS check failed for ${fullDomain}, assuming available`);
      return true;
    } catch (error) {
...
var DomainDatabase = class {
  static {
    __name(this, "DomainDatabase");
  }
  db;
  constructor(db) {
    this.db = db;
  }
  async saveSuggestion(name, industry, keywords, available) {
    try {
      await this.db.prepare(`
        INSERT OR REPLACE INTO domain_suggestions (name, industry, keywords, available, checked_at)
        VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)
      `).bind(name, industry, JSON.stringify(keywords), available).run();
    } catch (error) {
      console.error("Database save error:", error);
    }
  }
  async getSuggestions(industry, limit2 = 10) {
    try {
      const result = await this.db.prepare(`
        SELECT name, available FROM domain_suggestions
        WHERE industry = ? AND available = 1
        ORDER BY created_at DESC
        LIMIT ?
      `).bind(industry, limit2).all();
  ...
  async getIndustryPatterns(industry) {
    try {
      const result = await this.db.prepare(`
        SELECT patterns FROM industry_patterns WHERE industry = ?
      `).bind(industry).first();
      if (result?.patterns) {
        return JSON.parse(result.patterns);
      }
      return [];
    } catch (error) {
      console.error("Industry patterns query error:", error);
      return [];
    }
  }
};
var DomainCache = class {
  static {
    __name(this, "DomainCache");
  }
  cache;
  constructor(cache) {
    this.cache = cache;
...
var DomainSuggestionsService = class {
  static {
    __name(this, "DomainSuggestionsService");
  }
  ai;
  checker;
  db;
  cache;
  constructor(env) {
    this.ai = new AIDomainGenerator(env.OPENAI_API_KEY);
    this.checker = new DomainAvailabilityChecker();
    this.db = new DomainDatabase(env.DB);
    this.cache = new DomainCache(env.CACHE);
  }
  async generateSuggestions(request) {
    const startTime = Date.now();
    const { industry, keywords = [], limit: limit2 = 10, checkAvailability = true } = request;
    const cacheKey = `domains:${industry}:${keywords.sort().join(",")}:${limit2}:${checkAvailability}`;
    ...
    const existingSuggestions = await this.db.getSuggestions(industry, Math.floor(limit2 * 0.3));
    const aiSuggestions = await this.ai.generateDomains(industry, keywords, Math.ceil(limit2 * 0.7));
    const allNames = [.../* @__PURE__ */ new Set([
      ...existingSuggestions.map((s) => s.name),
      ...aiSuggestions
    ])].slice(0, limit2);
    const suggestions = [];
    for (const name of allNames) {
      ...
      if (checkAvailability) {
        const existing = existingSuggestions.find((s) => s.name === name);
        if (existing?.available !== void 0) {
          available = existing.available;
        } else {
          available = await this.checker.checkDomain(name);
          await this.db.saveSuggestion(name, industry, keywords, available);
        }
      }
      suggestions.push({
        name,
...
var src_default = {
  async fetch(request, env) {
    ...
    try {
      ...
      const service = new DomainSuggestionsService(env);
      if (url.pathname === "/api/suggest" && request.method === "POST") {
        ...
        const result = await service.generateSuggestions(body);
      ...
      if (url.pathname === "/api/suggest" && request.method === "GET") {
        ...
        const limit2 = parseInt(url.searchParams.get("limit") || "10");
        const checkAvailability = url.searchParams.get("checkAvailability") !== "false";
        if (!industry) {
...
// .wrangler/tmp/bundle-Pp6EdD/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Pp6EdD/middleware-loader.entry.ts
